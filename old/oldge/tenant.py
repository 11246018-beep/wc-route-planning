...
def is_local_demo_environment():
    env_name = str(os.environ.get("DJANGO_ENV") or os.environ.get("APP_ENV") or "").strip().lower()
    if env_name in {"production", "prod", "staging"}:
        return False
    if os.environ.get("DISPATCH_ALLOW_DEMO_DEFAULT", "").strip().lower() in {"1", "true", "yes", "on"}:
        return True
    return bool(getattr(settings, "DEBUG", False))


def resolve_company_key(explicit_key="", user=None, allow_demo_default=None):
    """
    Resolve the company key for scheduling/data access without ever falling back
    to a full service_points table scan.
    """
    key = str(explicit_key or "").strip()
    if key:
        return normalize_company_key(key), "API/環境變數傳入"

    env_key = str(os.environ.get("DISPATCH_COMPANY_KEY") or "").strip()
    if env_key:
        return normalize_company_key(env_key), "環境變數 DISPATCH_COMPANY_KEY"

    if getattr(user, "is_authenticated", False):
        company = get_user_company(user)
        if getattr(company, "key", ""):
            return normalize_company_key(company.key), "使用者目前公司"

    if allow_demo_default is None:
        allow_demo_default = is_local_demo_environment()
    if allow_demo_default:
        return DEFAULT_COMPANY_KEY, "本機/demo 預設"

    raise ValueError(
        "缺少 company_key。正式環境必須由 API、前端、使用者公司或 DISPATCH_COMPANY_KEY 明確指定，"
        "不可讀取 service_points 全表。"
    )
