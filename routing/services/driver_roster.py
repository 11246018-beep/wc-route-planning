...
def validate_driver_constraints(drivers, edit_driver_id, edit_driver_code, is_active, schedule_slot):
    ...
    for item in drivers:
        ...
        if int(cloned.get("id") or 0) == int(edit_driver_id or 0):
            ...
            cloned["schedule_slot_raw"] = schedule_slot
            cloned["schedule_slot_auto"] = False
        ...
        proposed.append(
            {
                ...
                "schedule_slot_raw": schedule_slot,
                "schedule_slot_auto": False,
            }
        )
...
def _slot_owner_priority(item, slot):
    """
    同一個席位如果資料髒掉，優先順序:
    1. 明確指定 schedule_slot 的司機
    2. 非自動席位
    3. 較新的資料（id / created_at 較大）
    4. 最後才用司機代碼排序
    """
    raw_slot = normalize_schedule_slot(item.get("schedule_slot_raw"))
    explicit_match = 1 if raw_slot == slot else 0
    is_auto = 1 if item.get("schedule_slot_auto") else 0
    item_id = int(item.get("id") or 0)
    created_sort = _timestamp_sort_value(item.get("created_at"))
    code = normalize_driver_code(item.get("driver_code"))
    return (
        -explicit_match,
        is_auto,
...
def build_schedule_driver_slots():
    ...
    for config in FIXED_SLOT_CONFIG:
        ...
        candidates = [
            ...
            if normalize_schedule_slot(item.get("schedule_slot")) == slot
        ]
        if not candidates:
            continue
        candidates.sort(key=lambda item: _slot_owner_priority(item, slot))
        active_by_slot[slot] = candidates[0]
