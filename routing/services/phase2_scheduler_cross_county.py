                ...
                slots.append(
                    {
                        ...
                        "tasks": [],
                        "seed_county": None,  # 只拿來排序偏好，不作硬限制
                    }
                )
    return slots
...
def candidate_score(route, task):
    ...
    # 沿用 normal 的想法：少開新槽位，並且偏好同 seed_county 先集中
    ...
    # 可跨縣市，但還是稍微偏好少混幾個縣市
    county_penalty = max(0, len(metrics["counties"]) - 1) * 3.0

    return metrics["total_min"] + new_slot_penalty + seed_bonus + county_penalty


def assign_cross_normal_core(tasks):
    ...
    for (depot_code, county), group_tasks in grouped_tasks:
        for task in group_tasks:
            ...
            rejected_tasks.append({**task, "reason": "no_540_slot_cross"})

    print(f"Cross strict unassigned tasks: {len(rejected_tasks)}")
...
def save_outputs(routes, flat_rows, rejected_tasks):
    ...
    payload = {
        "meta": {
            "variant": "cross",
            "label": "可跨縣市",
            "note": "沿用 normal 版核心分配邏輯，但移除同日不可跨縣市限制；固定84條、守540、允許 unassigned。",
        },
        "routes": routes,
    }
    ...
    (OUTPUT_DIR / "routes_cross_unassigned.json").write_text(
...
def main():
    ...
    route_slots, rejected_tasks = assign_cross_normal_core(tasks)
