...
def main():
    ...
    for _, row in df.iterrows():
        # 根據 weekly_1 和 weekly_2 計算一週內的總拜訪次數
        weekly_1 = int(row.get('weekly_1', 1)) if pd.notna(row.get('weekly_1')) else 1
        weekly_2 = int(row.get('weekly_2', 0)) if pd.notna(row.get('weekly_2')) else 0
        visits = max(1, weekly_1 + weekly_2)
        
        # 平分 service_time：每次拜訪使用相同的時間
        service_time_per_visit = row['Service_Time'] / visits
        ...
        for i in range(visits):
            tasks.append({
                ...
                'lat': row['Lat'],
                'lon': row['Lon'],
                'service_time': service_time_per_visit,  # 平分後的時間
                'county': row['County'],
                'depot': row['Depot'],
                'address': row['Address'],
                'visit_idx': i + 1,
    ...
    # ---------------------------------------
    # Export JSON for dashboard (requirement #1)
    print("Exporting route data to JSON...")
    # store at top-level /output instead of routing/output
    json_path = os.path.abspath(os.path.join(current_dir, '../../output/routes_new.json'))
    # make sure the directory exists (directory may not previously exist)
