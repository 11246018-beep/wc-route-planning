...
def main():
    ...
    for depot, num_drivers in driver_config.items():
        ...
        for driver_id in range(1, num_drivers + 1):
            ...
            for day in range(1, 7):
                ...
                est_day_total = 0
                while depot_unassigned:
                    def d(t): return (t['lat']-current_lat)**2 + (t['lon']-current_lon)**2
                    depot_unassigned.sort(key=d)
                    
                    cand = None
                    cand_idx = -1
                    best_est_time = 0
                    for idx, t_cand in enumerate(depot_unassigned):
                ...
                while current_route_tasks:
                    ...
                    if osrm_duration + total_service <= 540 or len(current_route_tasks) == 1:
                        ...
                        print(f"司機 {d_name} 第 {day} 天: {len(current_route_tasks)} 站, 總里程 {route_res['distance']:.1f} km, 車程 {osrm_duration:.1f} 分鐘, 總工時 {osrm_duration+total_service:.1f} 分鐘 (允許跨縣市)")
                        break
                    else:
    ...
    # ==========================================
    # If a driver has a day with only a few tasks or very short total time, we unassign it.
    # This creates COMPLETELY EMPTY days that the Final Pass can use to consolidate.
    driver_days_tasks = {}
    for s in schedule:
        k = (s['driver'], s['day'])
    ...
    for k, stasks in driver_days_tasks.items():
        ...
        day_total_time = last_node['travel_time_min'] + sum(x['service_time_min'] for x in stasks_sorted)
        
        # Dynamically unassign if the day is very short (<250 mins) or has <= 2 tasks
        if day_total_time < 250 or len(stasks) <= 2:
            for tk in stasks:
                tid = tk['task_id']
                for t in tasks_pool:
                    if t['task_id'] == tid:
                ...
                print(f"[CONSOLIDATION] Unassigned {tid} from {k[0]} Day {k[1]} (Underutilized: {day_total_time:.1f} mins, {len(stasks)} tasks) to free up the day.")
    ...
    if unassigned_pool:
        ...
        for t in unassigned_pool:
            ...
            for key, stasks in day_routes.items():
                ...
                if not stasks:
                    # EMPTY DAY!
                    # Penalize opening a new empty day, to encourage packing into fewer days
                    coords = [(depot_coords['lat'], depot_coords['lon']), (t['lat'], t['lon'])]
                    route_res = osrm.get_route_batch(coords)
                    osrm_duration = route_res['duration']
                    if osrm_duration + t['service_time'] <= 540:
                        extra_cost = osrm_duration + 10000 # ADD 10,000 PENALTY FOR NEW DAY
                ...
                # Cross-county enabled: Removed strict county checking.
                day_county = stasks_sorted[0]['county']
                if t['task_id'] == 'T01540' and key == ('W02', 1):
                    print(f"DEBUG {t['task_id']} on W02 Day 1. t_county={t['county']}, day_county={day_county} (Cross-County Enabled)")
    ...
    out_map = os.path.join(project_dir, 'output', 'Weekly_Routing_Map_CrossCounty_Compact.html')
    m.save(out_map)
    ...
    out_sched = os.path.join(project_dir, 'output', 'Weekly_Schedule_Summary_CrossCounty_Compact.xlsx')
    ...
    out_daily = os.path.join(project_dir, 'output', 'Daily_Route_Summary_CrossCounty_Compact.xlsx')
    daily_summary.to_excel(out_daily, index=False)
    
    print("Done! Saved Weekly_Schedule_Summary_CrossCounty_Compact.xlsx, Daily_Route_Summary_CrossCounty_Compact.xlsx and Weekly_Routing_Map_CrossCounty_Compact.html to output folder")
